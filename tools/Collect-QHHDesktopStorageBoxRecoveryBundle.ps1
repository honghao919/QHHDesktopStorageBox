...
param(
    ...
    [string[]]$DataDirectories = @(
        (Join-Path $env:LOCALAPPDATA 'QHHDesktopStorageBox'),
        'D:\WD'
    ),
...
$bundleName = 'QHHDesktopStorageBox-Recovery-{0:yyyyMMdd-HHmmss}' -f (Get-Date)
...
foreach ($dataDirectory in $DataDirectories | Select-Object -Unique) {
    ...
    Copy-BundleFileIfPresent -SourcePath (Join-Path $root 'qhhdesktopstoragebox.db') `
        -RelativeDestination (Join-Path $safeName 'qhhdesktopstoragebox.db')
    Copy-BundleFileIfPresent -SourcePath (Join-Path $root 'qhhdesktopstoragebox.db-wal') `
        -RelativeDestination (Join-Path $safeName 'qhhdesktopstoragebox.db-wal')
    Copy-BundleFileIfPresent -SourcePath (Join-Path $root 'qhhdesktopstoragebox.db-shm') `
        -RelativeDestination (Join-Path $safeName 'qhhdesktopstoragebox.db-shm')
...
Write-BundleText -RelativePath 'README.txt' -Value @(
    'This recovery bundle contains QHHDesktopStorageBox database copies, logs, volume information, and file metadata inventories.'
