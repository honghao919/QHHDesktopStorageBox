...
{
    private const string GitHubOwner = "honghao919";
    private const string GitHubRepo = "QHHDesktopStorageBox";
    ...
    private const string UpdateRootFolderName = "QHHDesktopStorageBoxUpdate";
    ...
    internal const string StartupSuccessMarkerEnvironmentVariable =
        "QHH_DESKTOP_STORAGE_BOX_STARTUP_SUCCESS_MARKER";

    private static readonly HttpClient HttpClient = new(new HttpClientHandler())
    {
        DefaultRequestHeaders =
        {
            { "User-Agent", "QHHDesktopStorageBox" }
        }
    };
        ...
        string? expectedSha256 = null)
    {
        ...
        try
        {
            ...
            updaterPath = Path.Combine(Path.GetTempPath(), $"QHHDesktopStorageBoxUpdater-{updateId}.bat");
            var updateLogPath = Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
                "QHHDesktopStorageBox",
                "Logs",
                "updater.log");
...
@echo off
setlocal
set "QHH_DESKTOP_STORAGE_BOX_ROLLBACK=%QHH_DESKTOP_STORAGE_BOX_UPDATE_ROOT%\rollback"
set "QHH_DESKTOP_STORAGE_BOX_EXIT_WAIT_SECONDS=30"
set "QHH_DESKTOP_STORAGE_BOX_STARTUP_WAIT_SECONDS=60"
>>"%QHH_DESKTOP_STORAGE_BOX_UPDATE_LOG%" echo [%date% %time%] Update started.

rem Wait for this exact process instance to exit naturally before replacing files.
if "%QHH_DESKTOP_STORAGE_BOX_APP_PID%"=="0" goto process_exited
for /l %%i in (1,1,%QHH_DESKTOP_STORAGE_BOX_EXIT_WAIT_SECONDS%) do (
    powershell.exe -NoProfile -Command "$process = Get-Process -Id %QHH_DESKTOP_STORAGE_BOX_APP_PID% -ErrorAction SilentlyContinue; if ($null -eq $process -or $process.StartTime.ToUniversalTime().Ticks -ne %QHH_DESKTOP_STORAGE_BOX_APP_START_TIME_UTC_TICKS%) { exit 0 }; exit 1" >nul 2>&1
    if not errorlevel 1 goto process_exited
    timeout /t 1 /nobreak >nul
)
>>"%QHH_DESKTOP_STORAGE_BOX_UPDATE_LOG%" echo [%date% %time%] Timed out waiting for the application process to exit.
exit /b 1

:process_exited
rem robocopy exit codes 0-7 are successful; 8 and above are failures.
robocopy "%QHH_DESKTOP_STORAGE_BOX_APP_DIR%" "%QHH_DESKTOP_STORAGE_BOX_ROLLBACK%" /E /COPY:DAT /DCOPY:DAT /R:1 /W:1 /NFL /NDL /NP >>"%QHH_DESKTOP_STORAGE_BOX_UPDATE_LOG%" 2>&1
...
robocopy "%QHH_DESKTOP_STORAGE_BOX_PAYLOAD%" "%QHH_DESKTOP_STORAGE_BOX_APP_DIR%" /E /IS /IT /COPY:DAT /DCOPY:DAT /R:1 /W:1 /NFL /NDL /NP >>"%QHH_DESKTOP_STORAGE_BOX_UPDATE_LOG%" 2>&1
if errorlevel 8 goto apply_failed

del /q "%QHH_DESKTOP_STORAGE_BOX_APP_DIR%\update.zip" "%QHH_DESKTOP_STORAGE_BOX_APP_DIR%\updater.bat" >nul 2>&1
del /q "%QHH_DESKTOP_STORAGE_BOX_STARTUP_SUCCESS_MARKER%" >nul 2>&1

powershell.exe -NoProfile -Command "$process = Start-Process -FilePath $env:QHH_DESKTOP_STORAGE_BOX_APP_EXE -WorkingDirectory $env:QHH_DESKTOP_STORAGE_BOX_APP_DIR -PassThru; $deadline = [DateTime]::UtcNow.AddSeconds([int]$env:QHH_DESKTOP_STORAGE_BOX_STARTUP_WAIT_SECONDS); while ([DateTime]::UtcNow -lt $deadline) { if (Test-Path -LiteralPath $env:QHH_DESKTOP_STORAGE_BOX_STARTUP_SUCCESS_MARKER -PathType Leaf) { exit 0 }; if ($process.HasExited) { exit 1 }; Start-Sleep -Milliseconds 200 }; if (-not $process.HasExited) { Stop-Process -Id $process.Id -Force -ErrorAction SilentlyContinue; $process.WaitForExit() }; exit 1" >>"%QHH_DESKTOP_STORAGE_BOX_UPDATE_LOG%" 2>&1
...
>>"%QHH_DESKTOP_STORAGE_BOX_UPDATE_LOG%" echo [%date% %time%] Update backup failed with exit code %QHH_DESKTOP_STORAGE_BOX_FAILURE_CODE%. Restarting the original installation.
set "QHH_DESKTOP_STORAGE_BOX_STARTUP_SUCCESS_MARKER="
start "" /b /d "%QHH_DESKTOP_STORAGE_BOX_APP_DIR%" "%QHH_DESKTOP_STORAGE_BOX_APP_EXE%" >nul 2>&1
if errorlevel 1 goto original_restart_failed
>>"%QHH_DESKTOP_STORAGE_BOX_UPDATE_LOG%" echo [%date% %time%] Original installation restarted after backup failure.
exit /b 1

:original_restart_failed
>>"%QHH_DESKTOP_STORAGE_BOX_UPDATE_LOG%" echo [%date% %time%] Failed to restart the original installation after backup failure.
exit /b 1

:apply_failed
>>"%QHH_DESKTOP_STORAGE_BOX_UPDATE_LOG%" echo [%date% %time%] Update apply failed with exit code %errorlevel%. Restoring the previous installation.
goto rollback

:start_failed
>>"%QHH_DESKTOP_STORAGE_BOX_UPDATE_LOG%" echo [%date% %time%] Updated application failed to start. Restoring the previous installation.

:rollback
rem Remove only files introduced by this payload that were absent from the backup.
powershell.exe -NoProfile -Command "$payload = $env:QHH_DESKTOP_STORAGE_BOX_PAYLOAD; $rollback = $env:QHH_DESKTOP_STORAGE_BOX_ROLLBACK; $app = $env:QHH_DESKTOP_STORAGE_BOX_APP_DIR; Get-ChildItem -LiteralPath $payload -Recurse -File | ForEach-Object { $relative = $_.FullName.Substring($payload.Length).TrimStart('\'); $target = Join-Path $app $relative; if (-not (Test-Path -LiteralPath (Join-Path $rollback $relative) -PathType Leaf) -and (Test-Path -LiteralPath $target -PathType Leaf)) { Remove-Item -LiteralPath $target -Force -ErrorAction Stop } }" >>"%QHH_DESKTOP_STORAGE_BOX_UPDATE_LOG%" 2>&1
if errorlevel 1 goto rollback_failed
robocopy "%QHH_DESKTOP_STORAGE_BOX_ROLLBACK%" "%QHH_DESKTOP_STORAGE_BOX_APP_DIR%" /E /IS /IT /COPY:DAT /DCOPY:DAT /R:1 /W:1 /NFL /NDL /NP >>"%QHH_DESKTOP_STORAGE_BOX_UPDATE_LOG%" 2>&1
if errorlevel 8 goto rollback_failed
>>"%QHH_DESKTOP_STORAGE_BOX_UPDATE_LOG%" echo [%date% %time%] Previous installation restored. Recovery files retained at "%QHH_DESKTOP_STORAGE_BOX_UPDATE_ROOT%".
set "QHH_DESKTOP_STORAGE_BOX_STARTUP_SUCCESS_MARKER="
start "" /b /d "%QHH_DESKTOP_STORAGE_BOX_APP_DIR%" "%QHH_DESKTOP_STORAGE_BOX_APP_EXE%" >nul 2>&1
if errorlevel 1 goto rollback_failed
exit /b 1
...
:rollback_failed
>>"%QHH_DESKTOP_STORAGE_BOX_UPDATE_LOG%" echo [%date% %time%] Update rollback failed. Recovery files retained at "%QHH_DESKTOP_STORAGE_BOX_UPDATE_ROOT%".
exit /b 1
"""";
    }
        ...
        long appProcessStartTimeUtcTicks)
    {
        ...
        startInfo.Environment["QHH_DESKTOP_STORAGE_BOX_UPDATE_ROOT"] = tempRoot;
        startInfo.Environment["QHH_DESKTOP_STORAGE_BOX_PAYLOAD"] = payloadDirectory;
        startInfo.Environment["QHH_DESKTOP_STORAGE_BOX_APP_DIR"] = appDirectory;
        startInfo.Environment["QHH_DESKTOP_STORAGE_BOX_APP_EXE"] = appExecutablePath;
        startInfo.Environment["QHH_DESKTOP_STORAGE_BOX_EXE_NAME"] = executableName;
        startInfo.Environment["QHH_DESKTOP_STORAGE_BOX_UPDATE_LOG"] = updateLogPath;
        startInfo.Environment["QHH_DESKTOP_STORAGE_BOX_APP_PID"] = appProcessId.ToString();
        startInfo.Environment["QHH_DESKTOP_STORAGE_BOX_APP_START_TIME_UTC_TICKS"] =
