...
internal static class SafeFileOps
{
    ...
    private const int DirectoryMoveRetryCount = 40;
        ...
        CancellationToken cancellationToken)
    {
        ...
        // ".name.qhhdesktopstoragebox-{guid}.tmp" staging file that only renames to the real name at the
    ...
    private static string CreateStagingPath(string destinationPath)
    {
        ...
        return Path.Combine(
            directory,
            $".{Path.GetFileName(destinationPath)}.qhhdesktopstoragebox-{Guid.NewGuid():N}.tmp");
    }
    ...
    private static string CreateHeldSourcePath(string sourcePath)
    {
        ...
        return Path.Combine(
            directory,
            $".{Path.GetFileName(sourcePath)}.qhhdesktopstoragebox-{Guid.NewGuid():N}.moving");
    }
