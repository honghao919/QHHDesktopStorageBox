...
public sealed class DesktopBoxManager
{
            ...
            for (var index = 0; index < boxes.Count; index++)
            {
                ...
                if (!_windows.TryGetValue(box.Id, out var window))
                {
                    var layoutSettings = new DesktopBoxLayoutSettings(
                        box.Type == QHHDesktopStorageBox.Core.Models.BoxType.Drawer);
    ...
    public async Task SaveAllPositionsAsync()
    {
        var positions = _windows
            .ToDictionary(
                pair => BoxPositionSettingPrefix + pair.Key.ToString("N"),
                pair => CaptureStoredPosition(pair.Value),
                StringComparer.Ordinal);

        await _drawerService.SetSettingsAsync(positions);
        _logger.Info($"Recorded complete desktop layout for {positions.Count} boxes.");
    }
