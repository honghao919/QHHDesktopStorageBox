...
{
    private const string SingleInstanceMutexName = @"Local\QHHDesktopStorageBox.SingleInstance";
    private const string SingleInstancePipeName = "QHHDesktopStorageBox.SingleInstance";
    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        if (ProcessElevation.RequiresUnelevatedRelaunch())
        {
            ...
            MessageBox.Show(
                $"QHH Desktop Storage Box 当前以管理员身份运行，Windows 会阻止桌面文件拖入盒子。\n\n"
                + $"自动切换到普通权限失败（错误码 {nativeErrorCode}）。请退出后直接双击启动，不要选择“以管理员身份运行”。",
                "QHH Desktop Storage Box 无法接收桌面拖放",
                MessageBoxButton.OK,
        ...
        catch (Exception exception)
        {
            ...
            MessageBox.Show(
                sb.ToString(),
                "QHH Desktop Storage Box startup failed",
                MessageBoxButton.OK,
    ...
    private void InitializeTaskbarIcon(AppPaths paths, IAppLogger logger)
    {
        ...
        _taskbarIcon = new TaskbarIcon(nint.Zero, iconPath, "QHH Desktop Storage Box");
        ...
        _taskbarIcon.RightClick += (_, _) =>
        {
            ...
            AppendMenuW(menu, 0, 3, "退出 QHH Desktop Storage Box");
    ...
    {
        var tempDir = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "QHHDesktopStorageBox");
