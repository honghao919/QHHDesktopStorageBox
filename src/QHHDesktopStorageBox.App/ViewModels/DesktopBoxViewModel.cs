...
public sealed class DesktopBoxViewModel : ObservableObject
{
    ...
    public bool SupportsRollUp =>
        Type is BoxType.Normal or BoxType.Pixel or BoxType.Mapping or BoxType.Inbox or BoxType.Smart;
    ...
    public async Task<IReadOnlyList<Guid>> ImportPathsAsync(IEnumerable<string> paths, int? startColumn, int? startRow)
    {
        ...
        if (pathList.Length == 0 || IsBusy || IsTodoBox || IsSmartBox)
        {
        ...
        try
        {
            ...
            var preflight = await _drawerService.PreflightImportAsync(BoxId, pathList);
            ...
            foreach (var path in pathList)
            {
                if (!IsFreeSort)
                {
                    ...
                    var sortedImport = await _drawerService.TryImportPathAsync(
                        BoxId,
                        path,
                        new ImportOptions(conflictPolicy));
                    if (sortedImport is not null)
                    {
                ...
                var importedItem = await _drawerService.TryImportPathAsync(
                    BoxId,
                    path,
                    new ImportOptions(conflictPolicy),
                    slot.Column,
                    slot.Row);
                if (importedItem is null)
                {
                    continue;
                }
        ...
        FileMovePreflightResult preflight)
    {
        ...
        {
            _logger.Error(exception, "Failed to request desktop import confirmation.");
            return ImportConflictPolicy.AutoRename;
        }
    ...
    public bool SupportsSorting =>
        Type is BoxType.Normal or BoxType.Pixel or BoxType.Mapping or BoxType.Drawer or BoxType.Inbox or BoxType.Smart;
