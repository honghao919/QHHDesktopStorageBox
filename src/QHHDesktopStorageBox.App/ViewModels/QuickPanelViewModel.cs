...
public sealed class QuickPanelViewModel : ObservableObject
{
    ...
    public string SearchText
    {
        ...
        set
        {
            ...
            {
                _ = ApplyFilterAsync(SearchDebounceMilliseconds);
            }
        }
    }
    ...
    public async Task LoadAsync()
    {
        ...
        try
        {
            ...
            await ApplyFilterAsync(delayMilliseconds: 0);
        }
    ...
    private async Task ApplyFilterAsync(int delayMilliseconds)
    {
        ...
        try
        {
            ...
            var matches = string.IsNullOrWhiteSpace(query)
                ? allItems.Take(300).ToList()
                : await Task.Run(
                    () => allItems
                        .Where(item => SearchTextNormalizer.Matches(
                            query,
                            item.DisplayName,
                            item.PathLabel,
                            item.BoxName))
            ...
            Items.ReplaceAll(matches);
            StatusText = $"{Items.Count} / {allItems.Count} 项";
        }
        ...
        {
            _logger.Error(exception, "Failed to filter quick panel items.");
            StatusText = "搜索失败";
        }
        finally
        {
            Interlocked.CompareExchange(ref _filterCts, null, cts);
            cts.Dispose();
        }
    }
