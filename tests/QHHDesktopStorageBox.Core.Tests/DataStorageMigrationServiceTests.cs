...
public sealed class DataStorageMigrationServiceTests
{
    ...
    public async Task MigrateAsync_CopiesDataAndWritesBootstrapConfig()
    {
        ...
        var targetRoot = Path.Combine(Path.GetTempPath(), "QHHDesktopStorageBox.Tests", Guid.NewGuid().ToString("N"));
        try
        {
    ...
    public async Task MigrateAsync_ReopenedServiceRepairsStoredPathsAndCanExportItem()
    {
        ...
        try
        {
            ...
            var normalBox = (await service.GetBoxesAsync()).Single(
                box => box.Type == QHHDesktopStorageBox.Core.Models.BoxType.Normal);
    ...
    public void StorageLocationStore_SaveAndLoadRoundTrips()
    {
        ...
        try
        {
            ...
            store.SaveConfiguredDirectory(@"D:\Data\QHHDesktopStorageBox");

            Assert.Equal(
                Path.GetFullPath(@"D:\Data\QHHDesktopStorageBox"),
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "QHHDesktopStorageBox.Tests", Guid.NewGuid().ToString("N"));
