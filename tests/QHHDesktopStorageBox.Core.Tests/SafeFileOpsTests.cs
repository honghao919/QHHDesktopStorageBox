...
public sealed class SafeFileOpsTests
{
    ...
    public void AreSameVolume_ReturnsTrueForPathsOnSameRoot()
    {
        ...
        var a = Path.Combine(root, "qhhdesktopstoragebox-a.txt");
        var b = Path.Combine(root, "nested", "qhhdesktopstoragebox-b.txt");
    ...
    public void CopyThenDelete_Directory_CopiesTreeAndRemovesSource()
    {
        ...
        Assert.Empty(Directory.GetDirectories(workspace.Root, "*.qhhdesktopstoragebox-*.tmp"));
        Assert.Empty(Directory.GetDirectories(workspace.Root, "*.qhhdesktopstoragebox-*.moving"));
    }
    ...
    public async Task CopyThenDelete_DirectoryTemporarilyLockedForRename_RetriesAndCompletes()
    {
        ...
        var stagingAppeared = await WaitForConditionAsync(
            () => Directory.GetDirectories(workspace.Root, ".target-dir.qhhdesktopstoragebox-*.tmp").Length > 0,
    ...
    public async Task CopyThenDelete_DirectoryChangesDuringCopy_PreservesUncopiedFiles()
    {
        ...
        var stagingChildAppeared = await WaitForConditionAsync(
            () => Directory
                .GetDirectories(workspace.Root, ".target-dir.qhhdesktopstoragebox-*.tmp")
    ...
    public void CopyThenDelete_File_LandsAtFinalNameWithoutStagingArtifacts()
    {
        ...
        var stagingArtifacts = Directory.GetFiles(workspace.Root, "*.qhhdesktopstoragebox-*.tmp");
    ...
    public void Move_DirectoryWithLockedFile_PreservesEntireSourceAndRemovesStaging()
    {
        ...
        Assert.False(Directory.Exists(targetDir));
        Assert.Empty(Directory.GetDirectories(workspace.Root, ".copied-dir.qhhdesktopstoragebox-*.tmp"));
        Assert.Empty(Directory.GetDirectories(workspace.Root, ".locked-dir.qhhdesktopstoragebox-*.moving"));
    }
    ...
    private sealed class TempWorkspace : IDisposable
    {
        ...
        {
            Root = Path.Combine(Path.GetTempPath(), "QHHDesktopStorageBox.SafeFileOps.Tests", Guid.NewGuid().ToString("N"));
...
public sealed class SafeFileOpsRollbackTests
{
    ...
    public void RestoreHeldSource_SourcePathWasRecreated_PreservesBothRecoveryCopies()
    {
        using var workspace = new TempWorkspace();
        var sourceDir = workspace.CreateDirectory("source-dir");
        var heldSourceDir = workspace.CreateDirectory(".source-dir.qhhdesktopstoragebox-test.moving");
        var stagingDir = workspace.CreateDirectory(".target-dir.qhhdesktopstoragebox-test.tmp");
    ...
    public void RestoreHeldSource_HeldPathDisappeared_PreservesVerifiedStagingCopy()
    {
        ...
        var heldSourceDir = Path.Combine(workspace.Root, ".source-dir.qhhdesktopstoragebox-test.moving");
        var stagingDir = workspace.CreateDirectory(".target-dir.qhhdesktopstoragebox-test.tmp");
    ...
    private sealed class TempWorkspace : IDisposable
    {
        public TempWorkspace()
        {
            Root = Path.Combine(Path.GetTempPath(), "QHHDesktopStorageBox.SafeFileOps.RollbackTests", Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(Root);
        }
