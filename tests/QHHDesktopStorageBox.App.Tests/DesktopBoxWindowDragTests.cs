...
public sealed class InternalDragCompletionTests
{
    ...
    public void Mark_WithSynchronousFlag_LeavesNoStaleEntriesForLaterDragOut()
    {
        ...
        DesktopBoxWindow.MarkDroppedInsideQHHDesktopStorageBox(payload);

        // 之后同一项目的新拖拽（新 DragId、同 ItemId）释放到桌面：不得命中任何残留。
        var dragOut = DesktopBoxWindow.DesktopBoxDragPayload.Create(itemId, sourceBoxId);
        Assert.False(DesktopBoxWindow.ConsumeDroppedInsideQHHDesktopStorageBox(dragOut));
        Assert.False(dragOut.WasDroppedInsideQHHDesktopStorageBox);
    }
    ...
    public void Mark_WithoutSynchronousFlag_FallbackEntryIsConsumedExactlyOnce()
    {
        ...
        Assert.True(DesktopBoxWindow.ConsumeDroppedInsideQHHDesktopStorageBox(first));
        Assert.True(first.WasDroppedInsideQHHDesktopStorageBox);

        var second = DesktopBoxWindow.DesktopBoxDragPayload.Create(itemId, sourceBoxId);
        Assert.False(DesktopBoxWindow.ConsumeDroppedInsideQHHDesktopStorageBox(second));
    }
}
