...
public sealed class DesktopBoxGridAlignmentTests
{
        ...
        BoxType type, string preset, bool titleVisible, bool namesVisible, bool fixedSize)
    {
        ...
        var paths = new AppPaths(Path.Combine(Path.GetTempPath(), "QHHDesktopStorageBoxLayout", Guid.NewGuid().ToString("N")));
    ...
    private static Window LoadWindow(DesktopBoxViewModel model, double dpi)
    {
        ...
        root.SetAttributeValue(XNamespace.Xmlns + "infra", "clr-namespace:QHHDesktopStorageBox.App.Infrastructure;assembly=QHHDesktopStorageBox.App");
        ...
        foreach (var element in root.DescendantsAndSelf())
        {
            ...
            {
                element.Name = XName.Get(element.Name.LocalName, element.Name.NamespaceName + ";assembly=QHHDesktopStorageBox.App");
            }
        }
